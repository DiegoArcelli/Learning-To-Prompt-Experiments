...
for c in range(num_classes):
    embeddings[c] = torch.load(f"./init_prompts/cifar100/class/prompt_{c}.pt")
...
plt.savefig(f'plots/similarity/cifar100_classes_similarity.png')
plt.clf()
# plt.show()
...
for t in range(num_tasks):
    embeddings[t] = torch.load(f"./init_prompts/cifar100/task/prompt_{t}.pt")
...
# cbar = hm.collections[0].colorbar
# cbar.ax.tick_params(labelsize=80)
plt.savefig(f'plots/similarity/cifar100_tasks_similarity.png')
plt.clf()