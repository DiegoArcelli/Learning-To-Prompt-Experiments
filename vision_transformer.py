...
from prompt import Prompt, TaskPrompt, ClassIncrementalPrompt, TaskIncrementalPrompt
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            top_k=None, batchwise_prompt=False, prompt_key_init='uniform', head_type='token', use_prompt_mask=False, class_inc=False, task_inc=False, frequency_penalization=False):
        """
        Args:
        ...
        self.task_inc = task_inc
        
        if prompt_length is not None and pool_size is not None and prompt_pool:

            num_tasks = num_classes // 10

            if self.class_inc:
                self.prompt = ClassIncrementalPrompt(
                ...
                self.prompt = TaskIncrementalPrompt(
                    length=prompt_length,
                    embed_dim=embed_dim,
                    prompt_init=prompt_init,
                    pool_size=pool_size,
                ...
                self.prompt = Prompt(
                    length=prompt_length, embed_dim=embed_dim, embedding_key=embedding_key, prompt_init=prompt_init,
                    prompt_pool=prompt_pool, prompt_key=prompt_key, pool_size=pool_size, top_k=top_k, batchwise_prompt=batchwise_prompt,
                    prompt_key_init=prompt_key_init, frequency_penalization=frequency_penalization
                )
            ...
            for task_id in range(num_tasks):
                task_test_counts.append({t: {k: 0 for k in range(pool_size)} for t in range(task_id+1)})
                class_test_counts.append({c: {k: 0 for k in range(pool_size)} for c in range((task_id+1)*10)})
            self.task_key_counts_test = task_test_counts
            self.class_key_counts_test = class_test_counts
            self.key_class_counts = {k: {c: 0 for c in range(num_classes)} for k in range(pool_size)}   
            self.key_task_counts = {k: {t: 0 for t in range(num_tasks)} for k in range(pool_size)}
    ...
    def forward_features(self, x, task_id=-1, cls_features=None, train=False):
        ...
        if hasattr(self, 'prompt'):
            ...
            res = self.prompt(x, prompt_mask=prompt_mask, cls_features=cls_features, train=train)
